...
export function PoiFormModal({ categoryId, zone, onClose, onSuccess }: { categoryId?: string, zone?: string, onClose: () => void, onSuccess: () => void }) {
  ...
  const editData = typeof window !== 'undefined' ? (window as any).editPoiData : null;
  ...
  // Clean editData on unmount
  useEffect(() => {
    return () => {
      if (typeof window !== 'undefined') {
        (window as any).editPoiData = null;
      }
    };
  }, []);
    ...
    <div className="fixed inset-0 z-100 flex items-center justify-center p-4 sm:p-0">
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden relative z-10 max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center p-6 border-b border-gray-100">
          <h2 className="text-xl font-bold text-gray-900">{formData.id ? 'Editar Punto de Interés' : 'Añadir Punto de Interés'}</h2>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <form id="poi-form" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-1">Foto Principal {!formData.id && '(Obligatoria)'}</label>
